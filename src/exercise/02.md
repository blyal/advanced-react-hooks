...
- React has three memoization techniques. Each only caches the most recent input
  and result

  - React.memo(): prevInput is props, prevResult is React Elements (JSX)
  - React.useMemo(function(), [dependencyArray]): prevInput is the dependency
    array, prevResult is what the function returns
  - React.useCallback(function, [dependencyArray]): prevInput is the dependency
    array, prevResult is the function
  - useCallback is just a shortcut to using useMemo for functions (so it's just
    a function which returns a function)
  - https://epicreact.dev/memoization-and-react

- Good to put static variables outside the function scope so they don't get
  recomputed every render
- So when to use useMemo and useCallback?

  1. Referential equality
  2. Computationally-expensive functions

  - https://kentcdodds.com/blog/usememo-and-usecallback

- In a useEffect that contains a function, instead of adding the function's
  variables to the dependency array, can be safer to add the function, which
  should be a useCallback function, to the dependency array
  - Adding the variables means if the function changes we also need to remember
    to update the useEffect
  - If we add a non–callbacked function to the d.a. instead, the useEffect will
    be called every render because functions defined inside our component are
    re–initialised every render
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=Advanced%20React%20Hooks%20%F0%9F%94%A5&e=02%3A%20useCallback%3A%20custom%20hooks&em=alexblyth%40pm.me).
