...
- useDebugValue is basically a way to put a label on your custom hooks, in case
  you have multiple uses of the same hook in a component and need to debug stuff
  - The label is only for viewing in the devTools (not used in production)
- This is only for custom hooks. useDebugValue won't work for putting a label in
  a function, for example
- If computing the debug value is expensive, can instead put the values in an
  object and use a second argument (callback function) to format the values (see
  extra credits)
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=Advanced%20React%20Hooks%20%F0%9F%94%A5&e=06%3A%20useDebugValue%3A%20useMedia&em=alexblyth%40pm.me).
