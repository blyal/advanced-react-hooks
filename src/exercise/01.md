...
- useReducer is like useState, but is built to allow taking an action on the
  current state rather than mutating/setting state directly
  ...
  - Outside of those "rules", everything is quite subjective
  - If states change together, it's probably better to keep them together in a
    reducer
    - But if something is quite independent, including it in a reducer will add
      unnecessary complexity
  - Start with useState and move to useReducer if notice that elements of state
    need to change together

- useReducer looks much like useState, but takes two args = second is the
  initial value, but the first is a callback function that takes two args itself
  = currentState and action or new state
  - Convention is to create the function seperately, outside of the component,
    unless using props in the callback
  - Other than the callback, useReducer works like useState (e.g. other syntax,
    dispatch function usage), but conventions are not identical
- Spreading two objects into one – shared properties will be overridden by the
  second object
  - Clever trick
- More good practice: allow reducer actions to receive an object or function
  (see tutorial)
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=Advanced%20React%20Hooks%20%F0%9F%94%A5&e=01%3A%20useReducer%3A%20simple%20Counter&em=alexblyth%40pm.me).
